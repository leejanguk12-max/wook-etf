  ...
  with st.spinner("실시간 시세 연산 중..."):
    ...
    if is_korean_market_hours:
      ...
      if base_nav_reference > 0:
        ...
        # [수정] 네이버 NAV 기준이 아니라, 실제 현재가격(current_etf_price) 대비 예상 iNAV의 변동률 계산
        if current_etf_price > 0:
          actual_vs_inav_pct = (
          ...
          inav_extra_info = f"<span style='color: {actual_vs_inav_color}; font-weight: normal;'>({actual_vs_inav_pct:+.2f}%)</span>"
        else:
          inav_extra_info = ""
        ...
        render_custom_metric(
            ...
            f"{diff_arrow} {diff_val:+,.0f} 원 {inav_extra_info} (기준 iNAV:"
