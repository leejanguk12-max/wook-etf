...
def get_tradingview_direct_prices(symbols):
    ...
    payload_stocks = {"symbols": {"tickers": tickers_to_query}, "columns": ["close", "change"]}
            ...
            if resp_stocks.status_code == 200:
                for item in resp_stocks.json().get("data", []):
                    ...
                    if ticker_clean not in result_map or result_map[ticker_clean][0] == 0.0:
                        result_map[ticker_clean] = (values[0] if values[0] is not None else 0.0, values[1] if values[1] is not None else 0.0)
...
if df_input is not None and not df_input.empty:
    ...
    with st.spinner("실시간 시세 연산 중..."):
        ...
        if is_korean_market_hours or not is_us_premarket_or_market_hours:
