...
def get_market_session_status():
    ...
    is_korean_market_hours = (not is_weekend) and (korean_market_open <= current_time_val <= korean_market_close)
    
    year = now_kst.year
    march_1 = datetime(year, 3, 1, tzinfo=ZoneInfo("Asia/Seoul"))
    second_sunday_march = 14 - (march_1.weekday() + 1) % 7
    dst_start = datetime(year, 3, second_sunday_march, 2, 0, tzinfo=ZoneInfo("Asia/Seoul"))
    
    nov_1 = datetime(year, 11, 1, tzinfo=ZoneInfo("Asia/Seoul"))
    first_sunday_nov = 7 - nov_1.weekday() if nov_1.weekday() != 6 else 7
    dst_end = datetime(year, 11, first_sunday_nov, 2, 0, tzinfo=ZoneInfo("Asia/Seoul"))
    
    is_dst = dst_start <= now_kst < dst_end
    premarket_start_val = (17 if is_dst else 18) * 60
    
    is_us_premarket_or_market_hours = (not is_weekend) and (current_time_val >= premarket_start_val or now_kst.hour < 9)
    
    return is_korean_market_hours, is_us_premarket_or_market_hours, now_kst, is_dst

def get_prev_business_day(ref_date):
    if ref_date.weekday() == 0: prev_day = ref_date - timedelta(days=3)
    elif ref_date.weekday() == 6: prev_day = ref_date - timedelta(days=2)
    elif ref_date.weekday() == 5: prev_day = ref_date - timedelta(days=1)
    else: prev_day = ref_date - timedelta(days=1)
    return prev_day.strftime("%Y-%m-%d")

def get_timefolio_constituents_by_date(idx=2, date_str=None):
    url = f"https://timeetf.co.kr/m11_view.php?idx={idx}&pdfDate={date_str}#constituentItems" if date_str else f"https://timeetf.co.kr/m11_view.php?idx={idx}#constituentItems"
    ...
    try:
        ...
        if date_input:
            ...
            if m: fetched_date = f"{m.group(1)}-{m.group(2)}-{m.group(3)}"

        if not fetched_date:
            m_sec = re.search(r'(\d{4})[.-/]\s*(\d{2})[.-/]\s*(\d{2})', (soup.select_one("#constituentItems") or soup).get_text())
            if m_sec: fetched_date = f"{m_sec.group(1)}-{m_sec.group(2)}-{m_sec.group(3)}"

        rows = soup.select("table tr")
        exclude_keywords = ["기준가", "비교지수", "초과성과", "종목", "총액", "합계", "TOTAL", "CODE", "DATE", "날짜", "일자"]
        ...
        for row in rows:
            ...
            if len(tds) >= 4:
                ...
                clean_ticker = "현금" if "현금" in raw_name or "예금" in raw_name or "CASH" in raw_name.upper() or "KRW" in raw_name.upper() else (raw_code.split()[0].strip().upper() if raw_code.split() else "")
                if not clean_ticker or clean_ticker in ["NAN", "NONE"] or any(kw == clean_ticker or kw in raw_code or kw in raw_name for kw in exclude_keywords) or re.match(r'^\d{4}[.-/]\d{2}[.-/]\d{2}$', clean_ticker):
                    continue
                    
                try:
                    weight_val = float(raw_weight.replace('%', '').replace(',', '').strip())
                    if 0 < weight_val <= 100: data.append({"종목코드": clean_ticker, "비중": weight_val, "평가금액": amt_val})
                except ValueError: pass

        if data:
            return pd.DataFrame(data).drop_duplicates(subset=["종목코드"]).reset_index(drop=True), fetched_date
    except Exception: pass
    return None, fetched_date

def get_naver_official_base_fx():
    try:
        resp = requests.get("https://finance.naver.com/marketindex/exchangeDetail.naver?marketindexCd=FX_USDKRW", headers={"User-Agent": "Mozilla/5.0"}, timeout=5)
        price_tag = BeautifulSoup(resp.text, "html.parser").select_one("p.no_today em")
        if price_tag: return float(price_tag.text.strip().replace(",", ""))
    except Exception: pass
    return 0.0

def get_naver_etf_market_data(ticker_code="426030"):
    result = {"current_price": 0.0, "prev_close": 0.0, "price_change_pct": 0.0, "naver_nav": 0.0, "naver_disparity": 0.0}
    try:
        resp = requests.get(f"https://finance.naver.com/item/main.naver?code={ticker_code}", headers={"User-Agent": "Mozilla/5.0"}, timeout=5)
        soup = BeautifulSoup(resp.text, "html.parser")
        
        today_tag = soup.select_one("p.no_today em span.blind")
        if today_tag: result["current_price"] = float(today_tag.text.strip().replace(",", ""))
            
        exday_text = (soup.select_one("div.rate_info") or soup).get_text()
        is_minus = bool(soup.select_one("p.no_exday em span.ico.down") or soup.select_one("em.no_down") or "하락" in exday_text)
            
        m_pct = re.search(r'([\+\-]?\d+\.\d+)\s*%', exday_text)
        if m_pct:
            val = float(m_pct.group(1))
            result["price_change_pct"] = -val if (is_minus and val > 0 and "-" not in m_pct.group(1)) else val
...
def get_timefolio_official_data(idx=2):
    ...
    try:
        resp = requests.get(f"https://timeetf.co.kr/m11_view.php?idx={idx}", headers={"User-Agent": "Mozilla/5.0"}, timeout=5)
        soup = BeautifulSoup(resp.text, "html.parser")
        for box in soup.select("div.standard_price_box") or soup.select("ul.price_info li") or soup.find_all("div"):
            box_text = box.get_text()
            if "실시간" in box_text and "기준가" in box_text:
                m_val = re.search(r'([\d,]+\.\d+)', box_text)
                if m_val: result["live_nav"] = float(m_val.group(1).replace(",", ""))
                m_time = re.search(r'(\d{4}[-.\/]\d{2}[-.\/]\d{2}\s+\d{2}:\d{2}:\d{2})', box_text)
                if m_time: result["live_time"] = m_time.group(1)
            elif "기준가" in box_text and "실시간" not in box_text:
                m_val = re.search(r'([\d,]+\.\d+)', box_text)
                if m_val: result["base_nav"] = float(m_val.group(1).replace(",", ""))
                m_date = re.search(r'(\d{4}[-.\/]\d{2}[-.\/]\d{2})', box_text)
                if m_date: result["base_date"] = m_date.group(1)
    except Exception: pass
    return result

# 🛠️ 프리마켓(premarket_close, premarket_change) 데이터를 포함하여 수집하도록 업그레이드된 함수
def get_tradingview_direct_prices(symbols):
    url = "https://scanner.tradingview.com/america/scan"
    clean_symbols = []
    for s in symbols:
        sym_str = str(s).split()[0].upper().replace("/", ".")
        if "NQU" in sym_str or "NQ1!" in sym_str or "NQ=" in sym_str: clean_symbols.append("QQQ")
        elif sym_str != "현금" and "현금" not in sym_str and "CASH" not in sym_str and "KRW" not in sym_str: clean_symbols.append(sym_str)
            
    clean_symbols = list(set(clean_symbols))
    tickers_to_query = [f"{ex}:{sym}" for sym in clean_symbols for ex in ["NASDAQ", "NYSE", "AMEX"]]
    
    # 프리마켓 전용 컬럼 추가 요청
    payload_stocks = {
        "symbols": {"tickers": tickers_to_query}, 
        "columns": ["close", "change", "premarket_close", "premarket_change"]
    }
    payload_fx = {"symbols": {"tickers": ["FX_IDC:USDKRW", "FX:USDKRW"]}, "columns": ["close"]}
    headers = {"User-Agent": "Mozilla/5.0"}
        ...
        if tickers_to_query:
            ...
            if resp_stocks.status_code == 200:
                for item in resp_stocks.json().get("data", []):
                    ...
                    values = item.get("d", [0.0, 0.0, 0.0, 0.0])
                    ...
                    pm_close = values[2] if len(values) > 2 and values[2] is not None else 0.0
                    pm_change = values[3] if len(values) > 3 and values[3] is not None else 0.0
                    
                    # 프리마켓 시간대이고 프리마켓 변동률 데이터가 유효하면 프리마켓 변동률 우선 반영
                    active_price = pm_close if (pm_close > 0) else close_price
                    active_change = pm_change if (pm_change != 0.0) else change_pct
                    
                    if ticker_clean not in result_map or result_map[ticker_clean][0] == 0.0:
                        result_map[ticker_clean] = (active_price, active_change)
                    
        resp_fx = requests.post("https://scanner.tradingview.com/forex/scan", json=payload_fx, headers=headers, timeout=5)
        if resp_fx.status_code == 200:
            data_fx = resp_fx.json().get("data", [])
            if data_fx and data_fx[0].get("d"):
                live_fx = data_fx[0]["d"][0] if data_fx[0]["d"][0] is not None else 0.0
    except Exception: pass
...
def color_change_pct(val):
    try:
        ...
        if val_num > 0: return 'color: #0055FF; font-weight: bold;'
        elif val_num < 0: return 'color: #FF0000; font-weight: bold;'
    except Exception: pass
    return ''
...
def color_weight_change(val):
    try:
        val_str = str(val)
        if "NEW" in val_str: return 'color: #8A2BE2; font-weight: bold;'
        if "OUT" in val_str: return 'color: #FF8C00; font-weight: bold;'
...
with col1: fetch_auto = st.button("🚀 자동 불러오기", use_container_width=True)
with col2: toggle_uploader = st.button("📁 엑셀 파일 수동 업로드하기", use_container_width=True)

if "show_uploader" not in st.session_state: st.session_state.show_uploader = False
if toggle_uploader: st.session_state.show_uploader = not st.session_state.show_uploader
    ...
    with st.spinner("실시간 시세 연산 중..."):
        ...
        if df_prev is not None and not df_prev.empty:
            for _, p_row in df_prev.iterrows():
                ...
                if p_code == "KRW": p_code = "현금"
                try: prev_weight_map[p_code] = float(str(p_row['비중']).replace('%', '').replace(',', '').strip())
                except ValueError: pass
        
        ticker_list = list(set(clean_df["종목코드"].tolist() + (df_prev['종목코드'].tolist() if df_prev is not None and not df_prev.empty else [])))
        ...
        if official_base_fx == 0.0: official_base_fx = live_fx
        usdkrw_change_pct = ((live_fx - official_base_fx) / official_base_fx) * 100 if official_base_fx > 0 else 0.0
        ...
        for _, row in clean_df.iterrows():
            ...
            if any(kw in ticker for kw in cash_keywords) or ticker == "KRW": ticker = "현금"
            try: weight = float(str(row["비중"]).replace('%', '').replace(',', '').strip())
        ...
        for ticker in ticker_list:
            ...
            prev_w = prev_weight_map.get(ticker, prev_weight_map.get("KRW", None) if ticker == "현금" else None)
            ...
            if prev_w is not None:
                ...
                if abs(w_diff) >= 0.001: w_diff_str = f"{w_diff:+.2f}%"
            else:
                w_diff_str = "✨ NEW"
                w_diff_val = weight
                if weight > 0: new_added_stocks.append(f"**{ticker}** ({weight:.2f}%)")
            ...
            if ticker == "현금":
                krw_amount = row_data.get("평가금액", 0.0) if row_data is not None and row_data.get("평가금액", 0.0) > 0 else 1.0
                live_data.append({"종목코드": "현금", "실시간 가격($)": krw_amount, "주가변동률(%)": 0.0, "당일비중(%)": weight, "전일비중(%)": prev_w_str, "비중변화(%)": w_diff_str, "비중변화_수치": w_diff_val})
            elif "NQU" in ticker or "NQ1!" in ticker or "NQ=" in ticker:
                qqq_price, qqq_change = batch_results.get("QQQ", (0.0, 0.0))
                live_data.append({"종목코드": f"{ticker} (QQQ대체)", "실시간 가격($)": qqq_price, "주가변동률(%)": qqq_change, "당일비중(%)": weight, "전일비중(%)": prev_w_str, "비중변화(%)": w_diff_str, "비중변화_수치": w_diff_val})
            elif ticker in batch_results and batch_results[ticker][0] > 0:
                live_price, stock_change_pct = batch_results[ticker]
                live_data.append({"종목코드": ticker, "실시간 가격($)": live_price, "주가변동률(%)": stock_change_pct, "당일비중(%)": weight, "전일비중(%)": prev_w_str, "비중변화(%)": w_diff_str, "비중변화_수치": w_diff_val})
            else:
                live_data.append({"종목코드": ticker, "실시간 가격($)": 0.0, "주가변동률(%)": 0.0, "당일비중(%)": weight, "전일비중(%)": prev_w_str, "비중변화(%)": w_diff_str, "비중변화_수치": w_diff_val})
        ...
        if current_etf_price > 0:
            ...
            st.markdown(f"<p style='font-size: 42px; font-weight: normal; margin-bottom: 0px; line-height: 1.2; color: #1f1f1f;'>{current_etf_price:,.0f} 원 {chg_str}</p>", unsafe_allow_html=True)
            st.markdown(f"<div style='display: inline-block; background-color: {'#ffebee' if disp_is_plus else '#e3f2fd'}; color: {'#c62828' if disp_is_plus else '#0277bd'}; padding: 2px 8px; border-radius: 12px; font-size: 14px; font-weight: 500; margin-top: 6px;'>{'↑' if disp_is_plus else '↓'} 실시간 괴리율: {naver_disp:+.2f}% {nav_text_part}</div>", unsafe_allow_html=True)
        
        st.markdown("<div style='margin-top: 20px;'></div>", unsafe_allow_html=True)

        is_korean_market_hours, is_us_premarket_or_market_hours, now_kst, is_dst = get_market_session_status()
        ...
        if is_korean_market_hours:
            ...
            def render_custom_metric(label, value, delta_text, is_plus):
                bg_color, text_color, arrow = ("#ffebee", "#c62828", "↑") if is_plus else ("#e3f2fd", "#0277bd", "↓")
                st.markdown(
                    f"""
                    <div style="margin-bottom: 10px;">
            ...
            render_custom_metric("📈 실시간 iNAV 추정 총 변동률", f"{total_inav_change:+.2f}%", f"주가({stock_inav_change:+.2f}%) + 환율({usdkrw_change_pct:+.2f}%)", total_inav_change >= 0)
            st.markdown("<div style='margin-top: 20px;'></div>", unsafe_allow_html=True)

            base_nav_reference = timefolio_data["base_nav"] if timefolio_data["base_nav"] > 0 else (naver_nav if naver_nav > 0 else naver_market["prev_close"])
            if base_nav_reference > 0:
                estimated_inav_price = base_nav_reference * (1 + (total_inav_change / 100))
                render_custom_metric("💵 나스닥100액티브(426030) 예상 iNAV", f"{estimated_inav_price:,.0f} 원", f"{estimated_inav_price - base_nav_reference:+,.0f} 원 (기준 iNAV: {base_nav_reference:,.0f}원)", estimated_inav_price >= base_nav_reference)

        # 🏛️ 공식 공시 기준가 및 포트폴리오 변동 내역 박스
        ...
        active_df['표시명'] = "<span style='font-size:20px; font-weight:bold;'>" + active_df['종목코드'] + "</span><br><span style='font-size:15px;'>" + active_df['주가변동률(%)'].map('{:+.2f}%'.format) + "</span>"
        ...
        stiler_top10 = display_top10.style.map(color_weight_change, subset=['비중변화(%)']).map(color_change_pct, subset=['주가변동률(%)']) if hasattr(display_top10.style, "map") else display_top10.style.applymap(color_weight_change, subset=['비중변화(%)']).applymap(color_change_pct, subset=['주가변동률(%)'])
        st.dataframe(stiler_top10.format({'당일비중(%)': '{:.2f}%', '주가변동률(%)': '{:+.2f}%'}).set_properties(**{'text-align': 'center'}), use_container_width=True, height=385)
        ...
        display_full_df = display_base_df[display_base_df['당일비중(%)'] > 0].copy().reset_index(drop=True)
        display_full_df.index = range(1, len(display_full_df) + 1)
        
        display_full_df['실시간 가격($)'] = display_full_df.apply(lambda r: f"{r['실시간 가격($)']:,.2f}" if r['실시간 가격($)'] > 0 else "-", axis=1)
        display_full_df['주가변동률(%)'] = display_full_df['주가변동률(%)'].apply(lambda x: f"{x:+.2f}%")

        cols_to_show = [c for c in ['종목코드', '실시간 가격($)', '주가변동률(%)', '당일비중(%)', '전일비중(%)', '비중변화(%)'] if c in display_full_df.columns]
        ...
        st.dataframe(styled_full.format({'당일비중(%)': '{:.2f}%'}).set_properties(**{'text-align': 'center'}), use_container_width=True, height=500)
