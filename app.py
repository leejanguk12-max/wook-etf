...
def get_naver_etf_market_data(ticker_code="426030"):
  ...
  try:
    ...
    nav_pattern = re.search(
        r"NAV[^<]*</t[dh]>\s*<t[dh][^>]*>\s*([\d,]+(?:\.\d+)?)\s*</t[dh]>",
        page_html,
        re.IGNORECASE,
    )
    if nav_pattern:
...
def get_timefolio_official_data(idx=2):
  ...
  try:
    ...
    # [수정] 타임폴리오 웹페이지의 구조(실시간 기준가 박스 vs 전일 기준가 박스)에 맞게 정확히 매칭
    for box in soup.select("div, span, li, p"):
      text = box.get_text(strip=True)

      # 1. 실시간 기준가 파싱 (예: 44,740.93 또는 48,996.83)
      if (
          ("실시간" in text or "자사" in text)
          and result["live_nav"] == 0.0
      ...
      # 2. 전일 확정 기준가 파싱 (기준가 (원))
      if (
          ("기준가" in text and "실시간" not in text and "자사" not in text)
          and result["base_nav"] == 0.0
          and any(char.isdigit() for char in text)
      ):
        m_base = re.search(
            r"([1-9]\d{,2}(?:,\d{3})+|\d{5,})(?:\.\d{2})?", text
        )
        if m_base:
  ...
  # 만약 파싱 실패 시 네이버 데이터로 안전 방어
  if result["live_nav"] == 0.0 or result["base_nav"] == 0.0:
...
if df_input is not None and not df_input.empty:
  ...
  with st.spinner("실시간 시세 및 15시 30분 회차 역추적 연산 중..."):
    ...
    if is_korean_market_hours:
      ...
      delta_detail_html = (
          f"{arrow_char} 주가: <span style='color: {main_theme_color};"
          ...
          f" {live_fx:,.2f}원 / 기준 환율: {fx_1530_base:,.2f}원)"
      )
      ...
      # [수정] 괴리율 계산에 사용된 네이버 NAV(`naver_nav`)를 기준 베이스로 연동
      base_nav_reference = (
          naver_nav
          if naver_nav > 0
          else (
