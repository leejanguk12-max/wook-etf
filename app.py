...
def get_tradingview_direct_prices(symbols):
  ...
  # 프리장/애프터마켓 시세 및 변동률 데이터(extended_hours) 요청 추가
  payload_stocks = {
    ...
    if tickers_to_query:
      ...
      if resp_stocks.status_code == 200:
        for item in resp_stocks.json().get("data", []):
          ...
          values = item.get("d", [None, None, None, None])
          ...
          # 프리장/애프터마켓 가격이 존재하고 0이 아니면 프리장 시세 적용, 그 외엔 정규장 시세 적용
          close_price = (
              ext_close
              if (ext_close is not None and ext_close > 0)
              else reg_close
          )
          change_pct = (
              ext_change if (ext_change is not None) else reg_change
          )
