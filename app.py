...
def get_timefolio_official_data(idx=2):
  ...
  try:
    ...
    soup = BeautifulSoup(resp.text, "html.parser")
    for box in soup.select("div, span, li, p"):
      text = box.get_text(strip=True)
      # [수정] 10,000원 이상의 올바른 NAV 금액 패턴만 안전하게 매칭 (30원 같은 오류 방지)
      m_val = re.search(r"([1-9]\d{,2}(?:,\d{3})+|\d{5,})\.\d{2}", text)
      if m_val:
        val = float(m_val.group(0).replace(",", ""))
        if 10000 <= val <= 200000:
          if "실시간" in text or ":" in text and result["live_nav"] == 0.0:
            if "실시간" in text or result["live_nav"] == 0.0:
              if result["live_nav"] == 0.0:
                result["live_nav"] = val
                m_time = re.search(
      ...
      m_base = re.search(r"([1-9]\d{,2}(?:,\d{3})+|\d{5,})\b", text)
      if m_base and result["base_nav"] == 0.0:
        val = float(m_base.group(1).replace(",", ""))
        if 10000 <= val <= 200000 and "기준가" in text and "실시간" not in text:
          result["base_nav"] = val
  ...
  # 만약 위 파싱에서 못 잡았을 경우 네이버 NAV로 안전하게 방어
  if result["live_nav"] == 0.0:
    naver_m = get_naver_etf_market_data("426030")
    if naver_m["naver_nav"] > 0:
      result["live_nav"] = naver_m["naver_nav"]
      result["base_nav"] = naver_m["naver_nav"]
...
def get_tradingview_direct_prices(symbols):
  ...
  payload_stocks = {
      ...
      "columns": ["close", "change", "premarket_close", "market"],
  }
  headers = {"User-Agent": "Mozilla/5.0"}
    ...
    if tickers_to_query:
      ...
      if resp_stocks.status_code == 200:
        for item in resp_stocks.json().get("data", []):
          ...
          if (
              pm_close > 0
              and "pre" in market_status
              and close_price - pm_close != 0
          ):
            active_price = pm_close
            active_change = ((pm_close - close_price) / close_price) * 100
          elif close_price > 0 and change_pct != 0:
            active_change = change_pct
  ...
  with st.spinner("실시간 시세 및 15시 30분 회차 역추적 연산 중..."):
    ...
    for ticker in ticker_list:
      ...
      if ticker == "현금":
        ...
        qqq_price, qqq_change = batch_results.get(ticker, (0.0, 0.0))
        live_data.append({
            "종목코드": f"{ticker} (QQQ대체)",
            "실시간 가격($)": qqq_price,
            "주가변동률(%)": qqq_change,
    ...
    if is_korean_market_hours:
      ...
      # [수정] 토탈 변동률(total_inav_change) 전체 방향에 맞춰 박스 색상을 통일
      ...
      delta_detail_html = (
          f"주가: <span style='color: {stock_color}; font-weight:500;'>{stock_inav_change:+.2f}%</span>"
          f" + 환율(15:30 기준 {fx_1530_base:,.2f}원 대비): <span"
          f" style='color: {fx_color}; font-weight:500;'>{usdkrw_change_pct:+.2f}%</span>"
      )
