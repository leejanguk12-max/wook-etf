...
def get_tradingview_direct_prices(symbols, is_premarket=False):
    ...
    if tickers_to_query:
      ...
      if resp_stocks.status_code == 200:
        for item in resp_stocks.json().get("data", []):
          ...
          # 프리마켓 시간대에는 프리마켓 종가(pm_close)와 전일 종가(close_price)를 비교한 변동률 사용
          if is_premarket and pm_close > 0 and close_price > 0:
            active_price = pm_close
            active_change = ((pm_close - close_price) / close_price) * 100
          else:
            # 본장 시간대 또는 프리마켓 데이터가 없을 때는 본장 정규장 변동률(change_pct) 사용
            active_price = (
                pm_close
                if (is_premarket and pm_close > 0)
                else (close_price if close_price > 0 else 0.0)
            )
...
if df_input is not None and not df_input.empty:
  ...
  # 현재 시장 상태 확인 (프리마켓 시간인지 여부 파악)
  is_korean_market_hours, is_us_premarket_or_market_hours, now_kst, is_dst = (
      get_market_session_status()
  )
  market_open_kst = (9 if is_dst else 10) * 65  # 미국 본장 시작 시간 판단용 (참고용)
  # 본장 시간 판별: 주말이 아니고, 서머타임 적용 시 22:30 ~ 익일 05:00, 미적용 시 23:30 ~ 익일 06:00
  us_market_open_hour = 21 if is_dst else 22
  us_market_open_minute = 30
  current_total_mins = now_kst.hour * 60 + now_kst.minute
  us_market_open_val = us_market_open_hour * 60 + us_market_open_minute
  us_market_close_val = (
      (5 if is_dst else 6) * 60
  )  # 익일 새벽 (단순 비교용 플래그)

  # 현재 시간이 미국 프리마켓 시간대인지 판별 (본장 시작 전인 오후 5시 ~ 밤 10시 30분 사이)
  premarket_start_hour = 17 if is_dst else 18
  is_strictly_premarket = (
      not now_kst.weekday() >= 5
      and (
          premarket_start_hour * 60
          <= current_total_mins
          < us_market_open_val
          or now_kst.hour < 9
      )
  ...
  with st.spinner("실시간 시세 및 15시 30분 회차 역추적 연산 중..."):
    ...
    # 세션 상태에 따라 프리마켓 변동률 계산 여부 전달
    batch_results = get_tradingview_direct_prices(
        ticker_list, is_premarket=is_strictly_premarket
    )
    ...
    if current_etf_price > 0:
      ...
      chg_str = (
          ...
          + ("#ffebee" if pct_is_plus else "#e3f2fd")
          + "; color: "
          + ("#c62828" if pct_is_plus else "#0277bd")
          + "; padding: 2px 8px; border-radius: 12px; font-size: 20px; font-weight:"
          f" normal; display: inline-block;'>({price_change_pct:+.2f}%)</span>"
      )
      ...
      st.markdown(
          ...
          + ("#ffebee" if disp_is_plus else "#e3f2fd")
          + "; color: "
          + ("#c62828" if disp_is_plus else "#0277bd")
          + "; padding: 2px 8px; border-radius: 12px; font-size: 14px;"
          " font-weight: 500; margin-top: 6px;'>"
          + ("↑" if disp_is_plus else "↓")
          + f" 실시간 괴리율: {naver_disp:+.2f}% {nav_text_part}</div>",
          unsafe_allow_html=True,
      )
    ...
    if is_korean_market_hours:
      ...
      def render_custom_metric(
        ...
        st.markdown(
            "<div style='margin-bottom: 10px;'>"
            + f"<div style='font-size: 14px; color: #6f727b; margin-bottom: 2px;'>{label}</div>"
            + f"<div style='font-size: 42px; font-weight: normal; color: #1f1f1f; line-height: 1.2; margin-bottom: 4px;'>{value}</div>"
            + f"<div style='display: inline-block; background-color: {bg_color}; color: {text_color}; padding: 2px 8px; border-radius: 12px; font-size: 14px; font-weight: 500;'>"
            + f"{arrow} {delta_text} {extra_info}</div></div>",
            unsafe_allow_html=True,
        )
      ...
      delta_detail_html = (
          ...
          f" + 환율(15:30 기준): <span style='background-color: {fx_bg}; color:"
      ...
      if base_nav_reference > 0:
        ...
        est_chg_str = (
            "<span style='background-color: "
            + ("#ffebee" if pct_is_plus_est else "#e3f2fd")
            + "; color: "
            + ("#c62828" if pct_is_plus_est else "#0277bd")
            + "; padding: 2px 8px; border-radius: 12px; font-size: 20px; font-weight:"
            f" normal; display: inline-block;'>({est_diff_pct:+.2f}%)</span>"
        )
        ...
        st.markdown(
            ...
            + "; padding: 2px 8px; border-radius: 12px; font-size: 14px;"
            " font-weight: 500; margin-top: 6px;'>"
            + ("↑" if pct_is_plus_est else "↓")
            + f" {est_diff_val:+,.0f} 원 (기준 iNAV: {base_nav_reference:,.0f}원)</div>",
            unsafe_allow_html=True,
        )
        st.markdown(
