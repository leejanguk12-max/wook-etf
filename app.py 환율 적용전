...
if df_input is not None and not df_input.empty:
    ...
    with st.spinner("실시간 시세 연산 중..."):
        ...
        if is_korean_market_hours:
            ...
            # 🛠️ 실시간 공식 기준가(live_nav)를 우선 베이스로 잡고, 없을 경우에만 전일 확정가(base_nav) 사용
            base_nav_reference = timefolio_data["live_nav"] if timefolio_data["live_nav"] > 0 else (timefolio_data["base_nav"] if timefolio_data["base_nav"] > 0 else (naver_nav if naver_nav > 0 else naver_market["prev_close"]))
        ...
        # 🏛️ 타임폴리오 공시 기준가 및 포트폴리오 변동 내역 박스
