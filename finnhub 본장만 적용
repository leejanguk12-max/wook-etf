...
def get_market_session_status():
    ...
    # 지연 대기조건 (프리장 개장 후 15분만 유지, 본장은 즉시 실시간)
    is_pre_delay_buffer = (not is_weekend_closed) and (
        premarket_start_val <= current_time_val < (premarket_start_val + 15)
    )
    is_reg_delay_buffer = False  # 본장은 지연 없이 즉시 실시간 연동

    # 본장 세션 판별 (개장 시각 ~ 다음날 아침 09:00)
    is_regular_session = False
    if not is_weekend_closed:
        if current_time_val >= reg_start_val or current_time_val < 9 * 60:
...
def fetch_finnhub_prices(symbols):
    """본장 진입 시 Finnhub REST API를 활용해 실시간 주가를 가져옵니다."""
    result_map = {}
    ...
    for sym in clean_symbols:
        try:
            ...
            if resp.status_code == 200:
                data = resp.json()
                c = data.get("c", 0.0)  # Current price
                pc = data.get("pc", 0.0)  # Previous close
                if c and pc and pc > 0:
                    chg_pct = ((c - pc) / pc) * 100
                    result_map[sym] = (float(c), float(chg_pct))
        except Exception:
            pass
...
def get_tradingview_direct_prices(symbols, is_regular_session=False):
    ...
    # 본장 세션일 때 Finnhub API 우선 활용
    finnhub_map = {}
    if is_regular_session:
        finnhub_map = fetch_finnhub_prices(symbols)
                ...
                for item in resp_stocks.json().get("data", []):
                    ...
                    if is_premarket_session:
                        if pre_close is not None and pre_close > 0:
                            ...
                            change_pct = (
                                pre_change if pre_change is not None else reg_change
                            )
                        else:
    ...
    # 본장이면 Finnhub 데이터 우선 오버라이드
    if is_regular_session and finnhub_map:
        for k, v in finnhub_map.items():
            result_map[k] = v
...
if df_input is not None and not df_input.empty:
    ...
    with st.spinner("실시간 시세 연산 중..."):
        ...
        batch_results, live_fx = get_tradingview_direct_prices(
            ticker_list, is_regular_session=is_regular_session
        )
...
""",
            ...
            delay_label = f"프리장 15분 지연데이터 대기 중 ⏳ ({pre_next_time}부터 제공)"
...
<div style="display: inline-block; background-color: #fff3e0; color: #e65100; padding: 2px 8px; border-radius: 12px; font-size: 13px; font-weight: 500;">
ℹ️ 미국 프리장 개장 후 15분간은 지연 시세 반영 대기 시간입니다.
</div>
</div>
""",
                unsafe_allow_html=True,
