    ...
    try:
        ...
        for box in soup.select("div.standard_price_box") or soup.select("ul.price_info li") or soup.find_all("div"):
            ...
            if "실시간" in box_text and "기준가" in box_text:
                ...
                m_time = re.search(r'(\d{4}[-.\/]\n?\d{2}[-.\/]\d{2}\s+\d{2}:\d{2}:\d{2})', box_text)
...
# 🛠️ 프리마켓 버그 원흉(premarket_change) 필드 배제 및 직접 수식 계산 적용 함수
def get_tradingview_direct_prices(symbols):
    ...
    # 꼬이는 premarket_change 대신 정밀 종가(close), 프리장 실시간가(premarket_close), 정규장 변동률(change)만 요청
    payload_stocks = {
...
if df_input is not None and not df_input.empty:
    ...
    with st.spinner("실시간 시세 연산 중..."):
        ...
        # 장중/대기 시간에는 변동률만 0.0으로 처리하고 비중 변화 데이터는 원본 유지
