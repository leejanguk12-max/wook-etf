...
def get_timefolio_official_data(idx=2):
  ...
  try:
    ...
    for box in soup.select("div, span, li, p"):
      ...
      # [수정] 타임폴리오 웹페이지 구조에 맞게 실시간/확정 NAV를 명확히 분리하여 파싱
      if "실시간" in text or ":" in text:
        m_val = re.search(
            r"([1-9]\d{,2}(?:,\d{3})+|\d{5,})\.\d{2}", text
        )
        if m_val and result["live_nav"] == 0.0:
      ...
      if "기준가" in text and "실시간" not in text:
        m_base = re.search(
            r"([1-9]\d{,2}(?:,\d{3})+|\d{5,})(?:\.\d{2})?", text
        )
...
if df_input is not None and not df_input.empty:
  ...
  with st.spinner("실시간 시세 및 15시 30분 회차 역추적 연산 중..."):
    ...
    if is_korean_market_hours:
      ...
      # [수정] 괴리율 구할 때 쓰던 네이버 NAV 값을 기준(베이스)으로 연동
      base_nav_reference = naver_nav if naver_nav > 0 else (timefolio_data["live_nav"] if timefolio_data["live_nav"] > 0 else naver_market["prev_close"])
