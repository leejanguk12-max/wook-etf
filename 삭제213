...
def get_naver_fx_by_reversing_charts():
  ...
  try:
    ...
    # 2. 네이버 차트 틱/분봉 데이터 전문 가져오기 (충분한 회차 확보)
    ...
    if resp_chart.status_code == 200:
      ...
      if isinstance(items, list) and len(items) > 0:
        # 최신 데이터부터 과거 방향(역순)으로 탐색하며 15:30 마감 시점 데이터 찾기
        for item in reversed(items):
          ...
          # 시간 문자열에 '15:30' 또는 '1530'이 포함되어 있으면 해당 회차를 15:30 마감가로 지정
          if ("15:30" in time_str or "1530" in time_str) and val > 0:
            fx_1530 = val
            break

        # 만약 정확한 15:30 타이밍을 못 찾았다면, 역순 탐색 중 15시 대에 찍힌 가장 마지막(마감에 가까운) 값을 임시로 사용
        if fx_1530 == 0.0:
          for item in reversed(items):
            time_str = str(
                item.get("localTime", "")
  ...
  if current_fx == 0.0:
    current_fx = 1470.0
  if fx_1530 == 0.0:
    fx_1530 = current_fx - 2.0  # 현재가와 구분을 위한 fallback
