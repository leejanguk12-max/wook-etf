...
def get_naver_fx_smart_reference():
  """네이버 차트 API에서 15:30 마감 환율과 현재 환율을 명확히 분리하여 가져옵니다."""
  current_fx = 0.0
  fx_target_1530 = 0.0
  headers = {"User-Agent": "Mozilla/5.0"}
    ...
    if resp_chart.status_code == 200:
      ...
      if isinstance(items, list):
        for item in items:
          ...
          if target_date_str in time_str.replace("-", "") and "15:30" in time_str:
        ...
        if fx_target_1530 == 0.0:
          for item in items:
            ...
            if target_date_str in time_str.replace("-", "") and "15:" in time_str:
              val = float(price_str) if price_str else 0.0
              if val > 0:
                fx_target_1530 = val
                break
  ...
  if fx_target_1530 == 0.0:
    try:
      ...
      prev_tag = soup_prev.select_one("p.no_today em")
      if prev_tag:
        fx_target_1530 = float(prev_tag.text.strip().replace(",", ""))
    except Exception:
      pass
    if fx_target_1530 == 0.0:
      fx_target_1530 = current_fx if current_fx > 0 else 1470.0

  if current_fx == 0.0:
    current_fx = fx_target_1530
