...
def get_naver_fx_1530_and_current():
  """네이버 금융 환율 차트 API를 통해 15시 30분 마감 환율과 현재 환율을 역추적합니다."""
  current_fx = 0.0
  fx_1530 = 0.0
  try:
    # 네이버 금융 환율 일봉/분봉 데이터 API (예시 심볼 USD/KRW)
    # chart/json.nhn 또는 통화 환율 모바일 API 활용
    url = "https://m.stock.naver.com/marketindex/price/FX_USDKRW"
    headers = {"User-Agent": "Mozilla/5.0"}
    ...
    if resp_chart.status_code == 200:
      ...
      if isinstance(items, list):
        target_diff = 99999
        for item in items:
          # 날짜 및 시간 필드 확인 (예: localDate 또는 30분 단위 시세 배열)
          time_str = str(
              item.get("localTime", "")
              or item.get("date", "")
              or item.get("time", "")
          ...
          if "15:30" in time_str and price_val > 0:
            fx_1530 = price_val
            break
          # 만약 정확한 15:30 문자열이 없으면 가장 가까운 시간대(오후 3시 30분 경) 탐색
          if "15:" in time_str and price_val > 0:
            fx_1530 = price_val

        if fx_1530 == 0.0 and items:
          # 만약 못 찾았을 경우 가장 최근 영업일의 종가나 리스트의 중간 마감값 활용
          fx_1530 = float(
  ...
  # 만약 API 실패 시 네이버 기본 상세 페이지에서 현재가라도 가져오기
  if current_fx == 0.0:
    try:
      resp = requests.get(
...
if df_input is not None and not df_input.empty:
  ...
  with st.spinner("실시간 시세 및 15시 30분 환율 역추적 연산 중..."):
    ...
    # 15시 30분 기준 환율 및 현재 환율 역추적 호출
    fx_1530_base, live_fx = get_naver_fx_1530_and_current()
    usdkrw_change_pct = (
    ...
    if is_korean_market_hours:
      ...
      # 주가 및 환율 각각의 변동률 배경색 적용 (상승: 빨간색 계열, 하락: 파란색 계열)
      ...
      delta_detail_html = (
          ...
          f" + 환율(15:30 기준 {fx_1530_base:,.2f원} 대비): <span"
