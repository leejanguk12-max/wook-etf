...
def get_naver_fx_smart_reference():
  """현재 시간(15시 30분 기준)에 맞춰 올바른 영업일의 15:30 마감 환율과 실시간 현재 환율을 가져옵니다."""
  current_fx = 0.0
  fx_target_1530 = 0.0
  headers = {"User-Agent": "Mozilla/5.0"}
  ...
  now_kst = datetime.now(ZoneInfo("Asia/Seoul"))
  current_time_val = now_kst.hour * 60 + now_kst.minute
  market_close_val = 15 * 60 + 30  # 15:30

  # 대상 날짜 결정: 15시 30분 이전이면 '어제(직전 영업일)' 15:30 기준, 이후면 '오늘' 15:30 기준
  target_date = now_kst
  if current_time_val < market_close_val:
    # 아직 오늘 15:30이 안 되었으므로 기준은 직전 영업일 마감가
    if target_date.weekday() == 0:  # 월요일 새벽/오전
  ...
  try:
    ...
    # 2. 네이버 차트 API에서 특정 날짜/시간(15:30) 부근의 환율 역추적
    chart_url = f"https://m.stock.naver.com/front-api/marketIndex/prices?category=exchange&code=FX_USDKRW&page=1&pageSize=600"
    ...
    if resp_chart.status_code == 200:
      ...
      if isinstance(items, list):
        for item in items:
          ...
          # 해당 날짜이면서 15:30 근처인 데이터 탐색
          if target_date_str in time_str.replace("-", "") and (
              "15:30" in time_str or "15:" in time_str
          ):
            fx_target_1530 = price_val
            break

        # 만약 정확한 매칭을 못 찾았을 경우 가장 안전한 최근 종가 대체
        if fx_target_1530 == 0.0 and items:
          fx_target_1530 = float(
  ...
  if current_fx == 0.0:
    try:
      ...
      price_tag = BeautifulSoup(resp.text, "html.parser").select_one(
          "p.no_today em"
      )
      if price_tag:
        current_fx = float(price_tag.text.strip().replace(",", ""))
    except Exception:
      pass

  if fx_target_1530 == 0.0:
    fx_target_1530 = current_fx

  return fx_target_1530, current_fx
...
if df_input is not None and not df_input.empty:
  ...
  with st.spinner("실시간 시세 및 스마트 15시 30분 환율 기준 연산 중..."):
    ...
    # 시간대에 맞춰 자동으로 15:30 기준점 설정 및 현재 환율 가져오기
    fx_1530_base, live_fx = get_naver_fx_smart_reference()
    usdkrw_change_pct = (
    ...
    if is_korean_market_hours:
      ...
      delta_detail_html = (
          ...
          f" + 환율(15:30 기준 {fx_1530_base:,.2f}원 대비): <span"
          f" style='background-color: {fx_bg}; color: {fx_color}; padding: 1px"
          f" 4px; border-radius: 4px;'>{usdkrw_change_pct:+.2f}%</span>"
      )
      ...
      if base_nav_reference > 0:
        ...
        render_custom_metric(
            ...
            f"{estimated_inav_price - base_nav_reference:+,.0f} 원 (기준 iNAV:"
