  ...
  try:
    ...
    for box in (
      ...
      if "실시간" in box_text and "기준가" in box_text:
        ...
        if m_date:
          result["base_date"] = m_date.get("value", "") or m_date.group(1)
  except Exception:
    pass
  return result
    ...
    if tickers_to_query:
      ...
      if resp_stocks.status_code == 200:
        for item in resp_stocks.json().get("data", []):
          ...
          # [수정] 본장(market_status에 pre가 없거나 정규장가 체결 중)일 때는 정규장 변동률(change_pct) 우선 적용
          # 프리마켓 시간대이거나 premarket_close가 유효할 때만 프리장 가격 적용
          ...
          if (
            ...
            # 본장 진입 후에는 트레이딩뷰가 제공하는 실시간 정규장 등락률(change)을 정확히 반영
            active_change = change_pct
